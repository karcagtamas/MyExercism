...
class DominoesTest {
    ...
    private class DominoListAsserter(private val outputDominoes: List<Domino>) {
        ...
        private fun haveSameDominoesAs(inputDominoes: List<Domino>) {
            val errorMessage =
                "The number of dominoes in the input list (${inputDominoes.size}) needs to match the number of dominoes in the output chain (${outputDominoes.size})"
            ...
            inputDominoes.forEach { domino ->
                ...
                val frequencyErrorMessage =
                    "The frequency of domino (${domino.left}, ${domino.right}) in the input is ($inputFrequency), but ($outputFrequency) in the output."
        ...
        private fun haveMatchingEnds() {
            ...
            val errorMessage =
                "The left value of the first domino ($leftValueOfFirstDomino) needs to match the right value of the last domino ($rightValueOfLastDomino)."
        ...
        private fun haveConsecutiveDominoes() {
            (0 until outputDominoes.size - 1).forEach { i ->
                ...
                val errorMessage =
                    "The right value of domino number $i ($rightValueOfIthDomino) needs to match the left value of domino number ${i + 1} ($leftValueOfNextDomino)."
